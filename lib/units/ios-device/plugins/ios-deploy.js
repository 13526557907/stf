...
.define(function(options){
  ...
  plugin.install=function(serial,file,type='device'){
    ...
    wdaPro.on("stream-line",line=>{
      console.log("this is line data2222");
      console.log(line);
      console.log(line.indexOf("failed"))
        if(line.indexOf("- Error occurred:")!=-1 || line.indexOf("failed")!=-1){
            this.emit('error',"上传失败！找不到此可执行文件的有效配置文件")
        }
        else if(line.indexOf('(')!=-1 && line.indexOf("failed")==-1){
