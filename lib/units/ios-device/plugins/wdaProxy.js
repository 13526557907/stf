...
.define(function(options,wda){
 ...
 // })
  ...
  plugin.startIproxy = function(localPort,remotePort){
    log.info("start iproxy with params:%d %d %s",localPort,remotePort,"-u",options.serial)
    pro = new SubpPocess("iproxy",[localPort,remotePort,"-u",options.serial])
    pro.start();
