  ...
  .define(function(options, adb, minicap, display, group, screenOptions) {
    ...
    FrameProducer.prototype._connectService = function(res) {
      ...
      group.getEmail().then(resp=>{
        _this.sendEmail = resp;
        this.sendEmail = resp;
      }) 
      this.intervalObj = setInterval(() => {
        console.log('这是_this.sendEmail');
        console.log(this.sendEmail);
          ...
          request.post({url:`${jwtutil.baseUrl}/stf/heartBeat`, form:{"username":encrypt, 'serialno':options.serial}}, function(error, response, body) {
            console.log("一分钟");
            console.log(error);
            if (!error) {
