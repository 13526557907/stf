...
var getUser = require('../../../../../res/app/components/stf/user/user-service');
...
var userFile = require('../../../../../lib/units/websocket/index')
...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, minicap, display, group, screenOptions) {
    ...
    FrameProducer.prototype.start = function() {
      ...
      group.getEmail().then(resp=>{
        this.sendEmail = resp;
        // sendTime(this.sendEmail,"1", 0)
        var encrypt =  jwtutil.encrypt(
          {  
            alg: 'des-ede3',    //3des-ecb  
            autoPad: true,  
            key: '0123456789abcd0123456789',  
            plaintext: this.sendEmail,   
            iv: null
        });
        var heartData = {"username":encrypt, 'serialno':options.serial,'heartType':1, 'debugId':0}
        request.post({url:`${jwtutil.baseUrl}/stf/heartBeat`, form:heartData}, function(error, response, body) {
          if (!error) {
            var respFirst = JSON.parse(body)
              console.log("-------111"+ respFirst.debugId)
              this.debugId = respFirst.debugId;
              debugId = respFirst.debugId;
        ...
        this.intervalObj = setInterval(() => {
          ...
          console.log(this.debugId);
          console.log(debugId)
          sendTime(this.sendEmail,"2",debugId)
        }, 1000 * 60);
      }) 
    ...
    FrameProducer.prototype.stop = function() {
      ...
      sendTime(this.sendEmail,"3",debugId)
    ...
    FrameProducer.prototype._connectService = function(res) {
      timeSecond = 0;
    ...
    function sendTime(data ,heartType, debugId) {           
        var encrypt =  jwtutil.encrypt(
          {  
            alg: 'des-ede3',    //3des-ecb  
            autoPad: true,  
        ...
        const _this = this;
        var heartData = {"username":encrypt, 'serialno':options.serial,'heartType':heartType, 'debugId':debugId}
        request.post({url:`${jwtutil.baseUrl}/stf/heartBeat`, form:heartData}, function(error, response, body) {
          if (!error) {
            if(heartType == 1) {
              console.log("-------111"+ respBody.debugId)
              _this.debugId = respBody.debugId;
              this.debugId = respBody.debugId;
              debugId = respBody.debugId;
              console.log(this.debugId);
            } else {
    ...
    FrameProducer.prototype._stop = function() {
      ...
      //   sendTime(timeSecond,this.sendEmail);
    ...
    function createServer() {
      ...
      console.log(screenOptions.publicPort);
      console.log(987544444444444444444444444444444444);
