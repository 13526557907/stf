...
, $location
  ...
  $scope.checkInput = function(brand, e, type , $index) {
    var object = {
      field: e.target.name,
      op: null,
      query: angular.lowercase(brand.name)
    }
    if(e.toElement.checked == true) {
     if(object.query == "apple") {
      $scope.phoneValues[1].values = $scope.filtersold;
       $scope.phoneSearchAndroid = 2;
       $scope.phoneSearchIos = 2;
     } else if(object.query !== "apple" && e.target.name == "manufacturer") {
       $scope.phoneValues[1].values = $scope.filtersold;
      $scope.phoneSearchAndroid = 1;
      $scope.phoneSearchIos = 1;
      angular.forEach($scope.phoneValues[1].values,function(subscription, index) {
        subscription.checked = false;
      })
      angular.forEach($scope.phoneValues[3].values,function(subscription, index) {
        subscription.checked = false;
      })
      $scope.filter = [{
        field: "manufacturer",
      ...
      angular.forEach($scope.phoneValues[$index].values, function(subscription, index) {
        if (type != index){
          subscription.checked = false;
          if($scope.filter.length>0) {
            for(var i=0;i<$scope.filter.length;i++) {
              if($scope.filter[i].field ==  e.toElement.name) {
      ...
      if(e.target.name == "manufacturer") {
        $scope.phoneValues[1].values = $scope.filtersnew;
        $scope.phoneSearchAndroid = 1;
        $scope.phoneSearchIos = 2;
        angular.forEach($scope.phoneValues[1].values,function(subscription, index) {
          subscription.checked = false;
    ...
    //   modelArr.push(value.name);
      
    // } else {
    //   let arrIndex = modelArr.indexOf(value.name);
    //   modelArr.splice(arrIndex,1);
    //   $scope.filter = QueryParser.parse(`${type}:`)
    //   modelArr.forEach(model=>{
    //     $scope.filter = QueryParser.parse(`${type}:${model}`)
    //   })
    // }
  }
  ...
  $scope.initfilters = function () {
    return $http.get('/api/v1/devices').then(function (response) {
      ...
      for (var i = 0; i < devices.length; i++) {
        ...
        //动态填充分辨率信息
        if (obj.display && obj.display.width != undefined) {
      ...
      $scope.filtersold = filters[1].values;
      $scope.filtersnew = filters[1].values = deviceFilterVersion(filters[1].values,'name');
      dispaly = sortByPublic(dispaly, 'x')
      filters[3].values = filters[3].values.concat(dispaly)

      filters[4].values = manufacturer
      $scope.phoneValues = filters;
      $scope.phoneValues[1].values = $scope.filtersnew;
      return filters
    })
 ...
 // name 查询条件传参选择的value，item当前对象
  ...
  function deviceFilterVersion(arr, key) {
    var result = [];
    var obj = {};
